$(function () {
    ...
    $('.request-date').text(employee_RequestDetails.startDate);
    $('.start-time').text(employee_RequestDetails.startTime);
    $('.end-time').text(employee_RequestDetails.endTime);
    $('.leave-type').text(employee_RequestDetails.leaveType);
    $('#noOfHours').text(employee_RequestDetails.numberOfHours);
...
function fnLoadRequest_Signed_Signatures() {
    ...
    ajaxRequest({
        commandName: 'Request_ShortLeave_History_Signed_Get',
        values: {
...
var responseCallBack = function (response) {
    ...
    swal(response.Value);
    setTimeout(function () {
        window.open('/Employees/Request/ShortLeaves', '_blank');
        window.close();
    }, 2000);
}
